  ...
  <div class="p-2.5 xl:p-5 flex justify-end">
    <div class="cart-totals">
      ...
      <div class="sub-total">
        ...
        <span class="value">{{ formatToRupiah(calculateTotalPrice()) }}</span>
      </div>

      <button class="checkout-btn" @click="checkout">Proceed to PR</button>
    </div>
  </div>
</template>
...
export default {
  ...
  setup() {
    ...
    const deleteCartItem = (itemId) => {
      ...
      axios
        .delete(
          `http://192.168.1.244:8000/api/buyer/removeCart/${itemId}`,
          config
        )
        .then(() => {
      ...
      if (index !== -1) {
        Swal.fire({
          ...
          if (result.isConfirmed) {
            ...
            axios
              .put(
                `http://192.168.1.244:8000/api/buyer/updateCart/${item.id}`,
