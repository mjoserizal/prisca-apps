    ...
    const getProductDetails = async () => {
      try {
        ...
        const response = await axios.get(
          `http://192.168.1.244:8000/api/buyer/show/product/${props.id}`,
          config
        );
      ...
      try {
        const { value: quantity } = await Swal.fire({
          ...
          inputValidator: (value) => {
            if (!value || value <= 0) {
              return "Mohon masukkan jumlah yang valid!";
            } else if (parseInt(value) > stock.value) {
              // Periksa apakah jumlah melebihi stok
              return `Jumlah tidak boleh melebihi stok (${stock.value}).`;
            }
          },
        });
        ...
        if (quantity) {
          ...
          const response = await axios.post(
            "http://192.168.1.244:8000/api/buyer/addToCart",
            data,
            config
          );
...
.product-description {
  padding: 20px;
  margin-top: -28px; /* Mengatur deskripsi sedikit di atas gambar */
  align-self: flex-start; /* Mengatur deskripsi agar sejajar dengan batas atas gambar */
}
...
@media screen and (max-width: 768px) {
  .product-details-section {
    ...
    align-items: center; /* Menengahkan vertikal di perangkat seluler */
  }
